...
contract Membership is Modifiers, ReentrancyGuard, OwnableInternal {
    using Math for uint256;

    event HANDLE_NFT_BUY_PROCESS(address indexed user_,uint256 new_NFT_,uint256 blocktime_);
    event HANDLE_NFT_UPGRADING_PROCESS(address indexed user_,uint256 old_NFT_, uint256 new_NFT_, uint256 blocktime_);
    event HANDLE_NFT_MINT_PROCESS(address indexed user_,uint256 new_NFT_,uint256 blocktime_);
        ...
        whenNotContract(msg.sender) 
    {
        ...
        if(tokenContract == address(0)){
            ...
            _transferAssets(false,ms.nft[ms.membership.nftContract][tokenID].price,user,tokenContract);
        }
        ...
        whenNotContract(msg.sender) 
    {
        ...
        if(ms.isStaker[user]){revert User_Is_Staker();}
        ...
        if(tokenContract == address(0)) {
            ...
            _transferAssets(true,hoopRemainingAmount,user,tokenContract);
        }else{
            uint256 hoopXremainingAmount = upgradeTokenPrice - tokenPrice;
            _transferAssets(true,hoopXremainingAmount,user,tokenContract);
        }
